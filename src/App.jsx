  ...
  return (
    <div className={`app-shell${isAdminLayout ? ' app-shell--admin' : ''}`}>
      ...
      {isAdminLayout ? (
        ...
        <main className={pathname === PATHS.login ? 'page-content-auth' : `page-content page-content--with-mobile-nav ${pathname === PATHS.home ? 'page-content-home' : [PATHS.event, PATHS.about, PATHS.club, PATHS.profile].includes(pathname) || clubUnitId ? 'page-content-wide' : (eventId || newsId ? 'page-content-full' : '')} ${unitStudentEventMatch ? 'page-content-unit-student' : ''}`}>
          {page}
        </main>
